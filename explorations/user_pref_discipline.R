...
#discipline.pref.usr.by.usr = aggregate(discipline_id ~ user_id, data.int.1, concat_tab)

#aggregate do data.table
data.int.1 <- data.table(data.int.1)
setkey(data.int.1, user_id)
discipline.pref.usr.by.usr = data.int.1[, list(value = concat_tab(discipline_id)), by = user_id]


head(discipline.pref.usr.by.usr, 10)

write.csv(discipline.pref.usr.by.usr, "/home/tales/development/recsys2016/data/interactions/discipline.pref.usr.by.usr.csv", quote=F, row.names = F)
