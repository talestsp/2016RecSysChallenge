...
#employment.pref.usr.by.usr = aggregate(employment ~ user_id, data.int.1, concat_tab)

#aggregate do data.table
data.int.1 <- data.table(data.int.1)
setkey(data.int.1, user_id)
employment.pref.usr.by.usr = data.int.1[, list(value = concat_tab(employment)), by = user_id]


head(employment.pref.usr.by.usr, 10)

write.csv(employment.pref.usr.by.usr, "/home/tales/development/recsys2016/data/interactions/employment.pref.usr.by.usr.csv", quote=F, row.names = F)
